...
angular.module('the-ng-jsdata').service('connect', function($q, DS, $timeout, onceAfterDigest, $ionicPopup, $state) {
  return (target, scope, callback, aggressive=true) => {
    ...
    const hasChanges = (record) => !_.isEqual(record.DSChanges(), { changed: {}, added: {}, removed: {} });
    ...
    unwatchers.push(scope.$on('$stateChangeStart', function(event, toState, toParams, fromState, fromParams) {
      ...
      if (_.find(records, hasChanges)) {
        ...
        $ionicPopup.confirm({ title: 'Discard changes?' }).then(
          (discard) => {
            if (discard) {
              _.each(records, (record) => record.DSRevert());
